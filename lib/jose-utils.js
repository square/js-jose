 ...
 * @return Promise<CryptoKey>
 */
export const importEcPublicKey = (ec_key, alg) => {
 ...
 * @return Promise<CryptoKey>
 */
export const importEcPrivateKey = (ec_key, alg) => {
  var config = webCryptographer.getSignConfig(alg);
  var usage = webCryptographer.getKeyUsageByAlg(alg);
...
export const sha256 = (str) => {
  ...
  return crypto.subtle.digest({name: "SHA-256"}, arrayFromString(str)).then(function(hash) {
