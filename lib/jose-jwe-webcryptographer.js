...
export class WebCryptographer {
  ...
  decrypt(cek_promise, aad, iv, cipher_text, tag) {
    ...
    if (config.auth.aead) {
      ...
      return cek_promise.then(function(cek) {
        var buf = Operations.arrayBufferConcat(cipher_text, tag);
  ...
  sign(aad, payload, key_promise) {
    ...
    return key_promise.then(function(key) {
      ...
      return this.crypto.subtle.sign(config.id, key, Operations.arrayFromString( base64UrlEncoder.encode(JSON.stringify(aad)) + '.' + base64UrlEncoder.encodeArray(payload)));
    });
  }
  ...
  verify(aad, payload, signature, key_promise, key_id) {
    ...
    return key_promise.then(function(key) {
      ...
      return this.crypto.subtle.verify(config.id, key, signature, Operations.arrayFromString(aad + "." + payload)).then(function(res) {
  ...
  keyId(rsa_key) {
    return Operations.sha256(rsa_key.n + "+" + rsa_key.d);
  }
  ...
  truncatedMac(config, mac_key_promise, aad, iv, cipher_text) {
    return mac_key_promise.then(function(mac_key) {
      var al = new Uint8Array(Operations.arrayFromInt32(aad.length * 8));
      var al_full = new Uint8Array(8);
      al_full.set(al, 4);
      var buf = Operations.arrayBufferConcat(aad, iv, cipher_text, al_full);
