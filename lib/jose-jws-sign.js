...
export class Signer {
  ...
  addSigner(rsa_key, key_id, aad, header) {
    var that = this;
    var key_promise;
    if (Operations.isCryptoKey(rsa_key)) {
    ...
    } else if (Operations.isCryptoKey(rsa_key)) {
  ...
  addSignature(jws, aad, header) {
    if (Operations.isString(jws)) {
      jws = JSON.parse(jws);
    }

    if (jws.payload && Operations.isString(jws.payload) &&
      jws.protected && Operations.isString(jws.protected) &&
      jws.header && jws.header instanceof Object &&
      jws.signature && Operations.isString(jws.signature)) {
  ...
  sign(payload, aad, header) {
    ...
    function sign (message, protectedHeader, unprotectedHeader, rsa_key_promise, kid) {
      ...
      if (Operations.isString(message)) {
        toBeSigned = Operations.arrayFromUtf8String(message);
      } else {
        try {
          toBeSigned = Operations.arrayish(message);
        } catch (e) {
          if (message instanceof JWS) {
            toBeSigned = Operations.arrayFromString(new Operations.Base64Url().decode(message.payload));
          } else if (message instanceof Object) {
            toBeSigned = Operations.arrayFromUtf8String(JSON.stringify(message));
          } else {
