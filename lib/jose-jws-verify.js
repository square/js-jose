...
JoseJWS.Verifier = function(cryptographer, message) {
  ...
  if (message.signature) {
    signatures.unshift({protected: {kid: protectedHeader.kid}, signature: message.signature});
  }

  that.signatures = {};
  for (var i = 0; i < signatures.length; i++) {
    that.signatures[signatures[i].protected.kid] = Utils.arrayFromString(Utils.Base64Url.decode(signatures[i].signature));
  }
