...
export class Verifier {
    ...
    signatures.forEach(function (sign) {
      ...
      sign.protected = JSON.parse(new Operations.Base64Url().decode(sign.protected));
    });

    that.aad = aad;
    protectedHeader = new Operations.Base64Url().decode(aad);
    try {
    ...
    for(var i = 0; i < signatures.length; i++) {
      ...
      that.signatures[i].signature = Operations.arrayFromString(new Operations.Base64Url().decode(signatures[i].signature));
    }
  ...
  addRecipient(rsa_key, key_id, alg) {
    ...
    var that = this,
      kid_promise,
      key_promise = Operations.isCryptoKey(rsa_key) ? new Promise(function (resolve) {
        resolve(rsa_key);
  ...
  verify() {
    ...
    signatures.forEach(function (sig) {
      ...
      promises.push(that.cryptographer.verify(sig.aad, that.payload, sig.signature, key_promises[kid], kid)
        .then(function (vr) {
          if (vr.verified) {
            vr.payload = new Operations.Base64Url().decode(that.payload);
          }
          return vr;
        }));
    });
