...
export class Verifier {
    ...
    that.base64UrlEncoder = new Utils.Base64Url();
    ...
    signatures.forEach(function (sign) {
      ...
      sign.protected = JSON.parse(that.base64UrlEncoder.decode(sign.protected));
    });

    that.aad = aad;
    protectedHeader = that.base64UrlEncoder.decode(aad);
    try {
    ...
    for(var i = 0; i < signatures.length; i++) {
      ...
      that.signatures[i].signature = Utils.arrayFromString(that.base64UrlEncoder.decode(signatures[i].signature));
    }
   ...
   */
  addRecipient(key, key_id, alg) {

    var that = this,
      kid_promise,
      key_promise = Utils.isCryptoKey(key) ? new Promise(function (resolve) {
        resolve(key);
      }) : Utils.importPublicKey(key, alg || that.cryptographer.getContentSignAlgorithm(), "verify");
    ...
    if (key_id) {
      ...
      kid_promise = Jose.WebCryptographer.keyId(key);
    }
  ...
  verify() {
    ...
    signatures.forEach(function (sig) {
      ...
      promises.push(that.cryptographer.verify(sig.aad, that.payload, sig.signature, key_promises[kid], kid)
        .then(function (vr) {
          if (vr.verified) {
            vr.payload = that.base64UrlEncoder.decode(that.payload);
          }
          return vr;
        }));
    });
